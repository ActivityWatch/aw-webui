...
div
  div.d-sm-flex.justify-content-between
    div
      h5.mb-2.mb-sm-0 {{ $t('settingsSections.activePattern') }}

      small.text-muted
        | {{ $t('settings.activePatternDescription') }}
        |
        | {{ $t('settings.activePatternDetails') }}
        |
        span.text-nowrap
    ...
    div
      ...
      small.text-right
        div.text-success(v-if="enabled && valid") {{ $t('settings.activePatternEnabled') }}
        div.text-danger(v-else-if="enabled") {{ $t('settings.activePatternInvalid') }}
        div.text-muted(v-else) {{ $t('settings.activePatternDisabled') }}
        div.text-danger(v-if="enabled && valid && broad_pattern") {{ $t('settings.activePatternTooBroad') }}
